...
#ENV MYSQL_PASSWORD 7z3DEBTa
#ENV DATABASE_NAME countryside
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/countryside-0.0.1.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/countryside-0.0.1.jar", "--spring.profiles.active=test"]